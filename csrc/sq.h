...
/*** Digital Camera support for ICRUISE ***/

int ioCameraAvailable();
int ioCameraStart(int slot, int width, int height, double zoom, int depth);
int ioCameraTakePicture(int slot, int *bits, int width, int height,
		    int buffering,
		    int bufferX, int bufferY,
		    int bufferW, int bufferH);

int ioCameraPeekPicture(int slot, int *bits, int width, int height);
int ioCameraRelease(int slot);
int ioCameraInFocus(int slot);


int sqZaurusCameraModel(int slot);
int sqZaurusCameraInitialize(int slot);
int sqZaurusCameraRelease(int slot);
int sqZaurusCameraSetSizeAndZoom(int slot, int width,
				 int height, int zoom);
int sqZaurusCameraSetLight(int slot, int light);
int sqZaurusCameraSetDirectToLCD(int slot, int direct, 
				 int left, int top,
				 int width, int height,
				 int zoom);
int sqZaurusCameraSetMode(int slot, int mode);
int sqZaurusCameraShutterButtonPressed(int slot);
int sqZaurusCameraInFocus(int slot);
int sqZaurusCameraWaitForShutter(int slot, int bits,
				 int bitsWidth,
				 int bitsHeight, 
				 int timeout);
int ioCameraCopy(int slot, int bits,
		 int bitsWidth, int bitsHeight);


/*** Power Management ***/
void ioPowerManagement(int allowPowerOff);
int ioPowerBatteryRemaining();
void ioFastTickUsage(int value);

/*** Zaurus Sound ***/
void sqZaurusSoundInitialize();
void sqZaurusSoundShutdown();
void sqZaurusSoundPrepareForRecording();
void sqZaurusSoundPlayVolume(int volume);
void sqZaurusSoundRecordVolume(int volume);
int sqZaurusSoundGetFileInfo(char *nameIndex, int nameSize,
			 int *format, int *samplingRate, int *channel, 
			 int *compressionScheme, int *dataSize, int *dataTime);
int sqZaurusSoundPlayFile(char *nameIndex, int nameSize,
			  int startByte, int endByte);
int sqZaurusSoundRecordFile(char *nameIndex, int nameSize,
			int format, int samplingRate,
			int compressionScheme, int sizeInByte);
void sqZaurusSoundPause();
void sqZaurusSoundContinue();
void sqZaurusSoundStop();
int sqZaurusSoundGetPosition();
int sqZaurusSoundReadyForAction(int action);
